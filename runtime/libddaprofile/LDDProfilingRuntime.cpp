...
int DDAProfilingFlag = 0; // Labeled entering the profiling scope and current ProfilingLoopID
...
void __addnewprofilingbuffer(char* FuncName, char* LoopPos)
{
  ...
  if( cur == end ){
    RWTraceInfo *HashNode;
    //HashNode = (RWTraceInfo*) malloc(sizeof(RWTraceInfo) * N);
    HashNode = new RWTraceInfo[N];
    HashRWTrace[DDAProfilingFlag] = HashNode; 
    MaxProfilingLoopID[DDAProfilingFlag] = DDAProfilingFlag;
    std::cout<< "new prof loop: " << DDAProfilingFlag << "  "<< FuncName << "  " << LoopPos << std::endl;
  }
  else{
  }
...
RWTraceInfo * lookupRWTraceInfo(size_t key, int *Addr)
{
  ...
  // Not insert any addr with key "key" before or inserted without hash conflictions.
  if((0 == HashRWTrace[DDAProfilingFlag][key].Addr) || ( HashRWTrace[DDAProfilingFlag][key].Addr ==  Addr))
    ...
    NewRWTraceInfo = HashRWTrace[DDAProfilingFlag][key].Next; // If there is no conflictions before, .Next = NULL.
...
bool operator< ( Dep LI1, Dep LI2)
{
  ...
  return ((LI1.SrcAddrPos < LI2.SrcAddrPos) || (LI1.SrcAddrPos == LI2.SrcAddrPos && LI1.SrcVarName < LI2.SrcVarName) || ( LI1.SrcAddrPos == LI2.SrcAddrPos && LI1.SrcVarName == LI2.SrcVarName && LI1.SinkAddrPos < LI2.SinkAddrPos) || ( LI1.SrcAddrPos == LI2.SrcAddrPos && LI1.SrcVarName == LI2.SrcVarName && LI1.SinkAddrPos == LI2.SinkAddrPos && LI1.SinkVarName < LI2.SinkVarName) );                                                                                          
}
  ...
  for(B = CurRWAddrNode->WTrace.begin(), E = CurRWAddrNode->WTrace.end(); B != E; ++B){
    ...
    for( IterNoB = B->RWIterNo.begin(),  IterNoE = B->RWIterNo.end(); IterNoB != IterNoE; ++IterNoB){
      ...
      // Loop carried true-dependence.
        ...
        std::map<Dep, std::set<int> >::iterator TBeg, TEnd, LCDB, LCDE;
        TEnd = LDDepResult[DDAProfilingFlag].LCDDep.TrueDep.end();
        //printf("LCDep: DDAProfilingFlag = %d \n", DDAProfilingFlag);
        //std::cout<< "Loop Carried True: "<< CurRWAddrNode->Addr<< ","<< B->AddrPos << ", "<<"SrcName " << B->VarName<<", " <<AddrPos << ","  << VarName <<" " << Dist<< std::endl;
        //std::cout<< "LoopCarriedTrue: " <<  DDAProfilingFlag << "addr " << CurRWAddrNode->Addr << std::endl;
        //std::cout<< "Loop Carried True: "<< CurRWAddrNode->Addr<< ","<< Deps.SrcAddrPos << ", "<<"SrcName " <<Deps.SrcVarName<<", " <<Deps.SinkAddrPos << ","  << Deps.SinkVarName <<" " << Dist<< std::endl;
        //LDDepResult[DDAProfilingFlag].LCDDep.TrueDep[Deps].insert(Dist);
         TBeg = LDDepResult[DDAProfilingFlag].LCDDep.TrueDep.find(Deps);
        if( TBeg != TEnd ){
          //LDDepResult[DDAProfilingFlag].LCDDep.TrueDep[Deps].insert(Dist);
          TBeg->second.insert(Dist);
         // printf("if size = %lu \n", LDDepResult[DDAProfilingFlag].LCDDep.TrueDep.size());
        }
        else{
         std::set<int> s1;
         s1.insert(Dist);
         LDDepResult[DDAProfilingFlag].LCDDep.TrueDep.insert(std::pair<Dep, std::set<int> > (Deps, s1 ));
        //printf("else size = %lu \n", LDDepResult[DDAProfilingFlag].LCDDep.TrueDep.size());
        }
        //printf("Deps_size = %lu \n", LDDepResult[DDAProfilingFlag].LCDDep.TrueDep[Deps].size());
        ...
        std::cout<< "name: " <<  LCDB->first.SrcAddrPos << ", " << LCDB->first.SrcVarName << ", " << LCDB->first.SinkAddrPos <<", " <<LCDB->first.SinkVarName << std::endl ;

        std::set<int>::iterator DB, DE;
        DB = LCDB->second.begin();
        DE = LCDB->second.end();
        for( ; DB != DE; DB++)
        std::cout<< "Dist = " << *DB << ", ";
